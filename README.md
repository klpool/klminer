...
支持自签名证书或者正规证书，支持安装为系统服务，开机自启动，支持进程守护运行，程序自动调整连接数限制。Telegram交流群 [点击加入](https://t.me/+cj19RtmLf00wNTBl) 。

## 关于我们

1. 软件抽水：0.1%
3. Telegram交流群 [点击加入](https://t.me/+cj19RtmLf00wNTBl) 。
...
7. 程序配置文件路径：`/etc/hellominer/conf`，可以通过修改`/etc/miner/conf/app.toml`里面的配置修改程序web管理端口。
...
1. 准备证书文件：  
程序默认自带了自签名证书，位于`/etc/miner/conf`目录下面，分别是证书文件`server.crt`和私钥`server.key`,
如果需要用自己的正规证书，只需要把你的证书改名成`server.crt`，私钥文件改成`server.key`。
覆盖`/etc/miner/conf`目录下面的同名文件即可。
